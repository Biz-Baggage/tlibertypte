    ...
    <footer className="mt-24 bg-primary text-primary-foreground">
      <div className="mx-auto grid max-w-7xl gap-10 px-4 py-14 md:grid-cols-4 md:px-8">
        <div>
          ...
          <p className="mt-2 text-xs text-primary-foreground/60">UEN: {company.uen}</p>
        </div>
        ...
        <div>
          ...
          <ul className="mt-4 space-y-2 text-sm">
            {focusAreas.map((f) => (
              <li key={f.slug} className="text-primary-foreground/80">
                {f.title}
              </li>
            ))}
          </ul>
        ...
        <div>
          ...
          <ul className="mt-4 space-y-3 text-sm text-primary-foreground/80">
            ...
            <li className="flex gap-2"><Mail className="mt-0.5 h-4 w-4 shrink-0" /><a href={`mailto:${company.email}`} className="hover:underline">{company.email}</a></li>
            <li className="flex gap-2"><Phone className="mt-0.5 h-4 w-4 shrink-0" /><a href={company.phoneHref} className="hover:underline">{company.phone}</a></li>
          </ul>
        </div>
      </div>
